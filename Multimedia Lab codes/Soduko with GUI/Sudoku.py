...
class Grid:
    ...
    def __init__(self, rows, cols, width, height, win):
        ...
        flag = solve_soduko(self.solved_state)
        if not flag:
            exit()
    ...
    def place(self, val):
        ...
        if self.cubes[row][col].value == 0:
            ...
            if self.solved_state[row][col] == val:
                return True
            else:
    ...
    def solve_gui(self):
        for i in range(self.rows):
            for j in range(self.cols):
                self.cubes[i][j].value = self.solved_state[i][j]
...
def redraw_window(win, board, time):
    ...
    text = fnt.render("PRESS SPACE TO SOLVE!", 1, (0, 0, 0))
